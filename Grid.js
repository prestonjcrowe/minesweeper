class Grid {
    ...
    drawCell(cell) {
        ...
        if (state > 0) {
            var c = color(this.NUM_COLORS[state - 1][0],
                          this.NUM_COLORS[state - 1][1],
                          this.NUM_COLORS[state - 1][2]);
            fill(c);
            text(state, x * this.cellSize + this.cellSize / 2,
    ...
    drawMine(x, y) {
        fill(244, 86, 66);
        rect(x * this.cellSize, y * this.cellSize,
            this.cellSize, this.cellSize);
    }

    drawFlag(x, y) {
        fill(158, 229, 91);
        ellipse(x * this.cellSize + this.cellSize / 2,
                y * this.cellSize + this.cellSize / 2,
